...
export function saveSettings(userId: string, data: unknown) {
  ...
  // The same gate as the private profile: an account with no stated adult year must not publish
  // its display name on a board every visitor can read.
  if (!isAdult(statedYear)) { persisted.privacy = 'private'; persisted.leaderboards = false; }
  ...
  if (!persisted.leaderboards) run('DELETE FROM leaderboards WHERE user_id=?', userId);
