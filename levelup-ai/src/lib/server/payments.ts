...
export const paymentAdapter: PaymentAdapter = {
  create(userId, input) {
    ...
    else { const listing = one('SELECT * FROM marketplace_paths WHERE id=? AND status=? AND deleted_at IS NULL', value.marketplacePathId!, 'approved'); assert(listing && listing.price > 0, 404, 'המסלול אינו זמין לרכישה. / Path is not available for purchase.'); assert(listing.creator_id !== userId, 409, 'המסלול הזה שלך, אין צורך לרכוש אותו. / This is your own path; no purchase is needed.', 'ALREADY_OWNED'); assert(!one('SELECT id FROM marketplace_sales WHERE buyer_id=? AND marketplace_path_id=?', userId, listing.id), 409, 'המסלול כבר נרכש. / You already own this path.'); amount = listing.price; }
